...
class BankAccount:
    ...
    def deposit(self, amount):
        self.balance += amount
        print(f"{self.balance}원 입금 완료. 현재잔액 : {self.balance}원")
    ...
    def withdraw(self, amount):
        if self.balance <= amount:
            ...
            return 
        self.balance -= amount
        print(f"{amount}원 출금 완료. 현재잔액 : {self.balance}원")
           
    def show_balance(self):
        return print(f"계좌 : {self.account_number}\
                      잔액 : {self.balance}원")
...
class Product:
    ...
    def reduce_stock(self, quantity):
        if self.__stock >= quantity:
            ...
            print(f"{self.name} 재고 {quantity}\
                   차감 남은수량 : {self.__stock}")
        else:
            print(f"{self.name} 재고 부족.")

    def add_stock(self, quantity):
        self.__stock += quantity
        print(f"{self.name} 재고 {quantity} 추가, 남은수량 : {self.__stock}")
...
class Cart:
    def __init__(self):
        self.items = []     # Cart(장바구니) 객체 생성시 리스트는 비어있음
    ...
    def add_product(self, product, quantity):
        ...
        if product.get_stock() >= quantity:
            ...
            print(f"{product.name}은 재고 부족으로 장바구니에 담을수 없습니다.")
    ...
    def show_cart(self):
        ...
        print(f"총금액 : {self.total_price}원")
