...
def practice2():
  ...
  # => if else 문은 반복문 앞쪽에 붙혀야함
  print(data)

  words = ['animals', 'foods', 'tools', 'happy', 'sad']
  # 위 단어들 중 글자수가 5자인 항목만 대문자로 변경(upper)
...
def practice4():
  ...
  # images = [f for f in files if re.search('g', f)]
  # images = [f for f in files if ('.png' in f or '.jpg' in f or '.jpg' in f)]
  images = [f for f in files if f.endswith(('.png', '.jpg', '.jpeg'))]
  print(f'이미지 파일 : {images}')
